...
const PrimaryTab: React.FC<TabsProps> = ({ tabs, onTabChange }) => {
  ...
  const handleTabClick: (key: string) => void = (key: string) => {
    setActiveTab(key);
    onTabChange?.(key);
  };
  ...
  return (
    <div className="flex flex-col md:flex-row md:gap-8 bg-white/5 rounded-3xl border-2 border-white/10 p-4 md:p-6 md:py-8 h-auto md:h-[300px] lg:h-[360px] overflow-y-auto hide-scrollbar relative z-100">
      {/* Left Tabs Container */}
      <div
        ref={leftTabsContainerRef}
        className="grid grid-cols-2 md:flex flex-col items-center w-full md:w-1/3 gap-4 md:gap-0 md:space-y-6 p-4"
      >
      ...
      <div ref={contentSectionRef} className="flex-1 p-4 md:px-6">
        ...
        {/* <header
          className="-mx-3 text-primary-orange text-[16px] lg:text-[24px] font-semibold leading-[1.2]"
        ...
        <Typography
          as="h4"
          className="text-primary-orange text-center md:text-left font-semibold"
        >
          {tabs[activeTab].title}
        </Typography>

        {/* <article className="my-6 text-white prose prose-invert max-w-none">
          {tabs[activeTab].description}
        </article> */}
        <Typography as="p" className="my-6 max-w-none">
          {tabs[activeTab].description}
        </Typography>
      </div>
    </div>
